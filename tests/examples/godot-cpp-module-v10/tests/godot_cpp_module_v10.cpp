...
int main() {
    ...
    // These bindings are Godot 4.6. The 4.5 member checks GODOT_VERSION_MAJOR
    // / _MINOR, but those are MACROS from version.hpp and a module cannot
    // export macros -- so the version is asserted through types instead:
    // EditorDock and AHashMap exist in 4.6 and not in 4.5.
    const bool version_ok = sizeof(EditorDock) > 0 &&
                            sizeof(AHashMap<int, int>) > 0;
    ...
    const bool ok = version_ok && vec2_ok && vec3_ok && basis_ok && color_ok &&
                    aabb_ok && gen_ok && math_ok;
    std::println("version={} vec2={} vec3={} basis={} color={} aabb={} gen={} math={}",
                 version_ok, vec2_ok, vec3_ok, basis_ok, color_ok, aabb_ok, gen_ok, math_ok);
    return ok ? 0 : 1;
}
